 ...
 * @return void
 */
function _unlink($data){
    $_count=count($data);
    for($i=0;$i<$_count;$i++){
        @$file='Public/Uploads/'.$data[$i];
        @$image='Public/image/'.$data[$i];
        @unlink($file);
        @unlink($image);
    }
}
...
/**
 * _Uploads()上次多个图片
 * @param string $_img_name 图片名字
 * @param string $_img_route 图片路径
 * @return array
 */
function _Uploads($_img_name,$_img_route){
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        if ($tmpFilePath != "") {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $splitNames = explode(".", $_FILES['file']['name'][$i]);//将文件原名按.分割打成字符串数组
                unset($splitNames[count($splitNames)-1]); //抹掉文件后缀名数据
                $data[$_img_name][$i] = implode('.', $splitNames);//将抹掉后缀名的数组拼接成文件名
